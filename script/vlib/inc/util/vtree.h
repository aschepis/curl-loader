...
V_INLINE void VTREE_merge_childs(VTREENODE *parent, VTREE_INSERT_MODE mode, VTREENODE *newnode)
{	
	VTREENODE *ch;
	...
	case VTREE_INSERT_LAST:	
		ch = VTREE_first_child(newnode);
		if (!ch) {
			return;
		}

		while(ch) {
			VTREE_insert_child(parent, ch, VTREE_INSERT_LAST, 0 );
			ch = VTREE_right_sibling(ch);
		}
		break;

	case VTREE_INSERT_FIRST:	
		ch = VTREE_last_child(newnode);
		if (!ch) {
			return;
		}

		while(ch) {
			VTREE_insert_child(parent, ch, VTREE_INSERT_FIRST, 0 );
			ch = VTREE_left_sibling(ch);
		}
		break;
	}
}
...
typedef int  (*VTREE_VISITOR_EXT_V) (VTREENODE *entry, VTREE_VISIT_EVENT visit, void *context);


#define VTREE_FOREACH_CHILD(current, node) \
	for((current) = VTREE_first_child(node); \
		(current); \
		(current) = VTREE_right_sibling(current)) 

#define VTREE_FOREACH_CHILD_REVERSE(current, node) \
	for((current) = VTREE_last_child(node); \
		(current); \
		(current) = VTREE_left_sibling(current)) 


V_INLINE size_t VTREE_count_child_nodes( VTREENODE *current)
{
	size_t ret = 0;
	VTREENODE *c;

	VTREE_FOREACH_CHILD(c,current) {
		ret++;
	}
	return ret;
}
...
/*
 ...
 c e-f
*/
V_INLINE VTREENODE *VTREE_preorder_next(VTREENODE *current)
{
...
#if 0
V_INLINE VTREENODE *VTREE_preorder_next_ext(VTREENODE *current, VTREE_VISITOR_EXT_V ext, void *context)
{
	if (current->nextlevel_first) {
		current = current->nextlevel_first;
	} else {
		if (current->right) {
			...
			while(current->parent) {
				...
				ext( context, VTREE_POSTORDER, context);
 ...
 */
V_INLINE void VTREE_foreach_child( VTREENODE *lst, VTREE_VISITOR_V eval, void *context)
{
    VTREENODE *cur;
...
V_INLINE void VTREE_foreach_child_reverse( VTREENODE *lst, VTREE_VISITOR_V eval, void *context)
{
    VTREENODE *cur;
	...
	VTREE_FOREACH_CHILD_REVERSE( cur,lst) {
	   	eval( cur, context );
	}
 ...
 *
 */
V_INLINE VTREENODE * VTREE_findif_child( VTREENODE *tree, VTREE_VISITOR eval, void *context, V_INT32 *retval)
{
	int ret;
    VTREENODE *cur;
...
V_INLINE VTREENODE * VTREE_findif_child_reverse( VTREENODE *tree, VTREE_VISITOR eval, void *context, V_INT32 *retval)
{
	int ret;
    VTREENODE *cur;
	...
	VTREE_FOREACH_CHILD_REVERSE( cur, tree) {
	   	ret = eval( cur, context );
	   	if (ret) {
			if (retval) {
				*retval = ret;
...
/*
 ...
 a b-c
*/
V_INLINE VTREENODE *VTREE_postorder_next(VTREENODE *current, VTREENODE *prev)
{
 ...
 */
V_INLINE void VTREE_foreach_preorder(VTREENODE *node, VTREE_VISITOR_V visit, void *context)
{
	VTREENODE *current;
...
V_INLINE VTREENODE * VTREE_find_preorder(VTREENODE *node, VTREE_VISITOR visit, void *context)
{
	VTREENODE *current;
 ...
 */
V_INLINE void VTREE_foreach_postorder(VTREENODE *node, VTREE_VISITOR_V visit, void *context)
{
	VTREENODE *current;
...
#if 0
V_INLINE void VTREE_foreach_preorder_ext(VTREENODE *tree, VTREE_VISITOR_EXT_V visit, void *context)
{
	VTREENODE *current;
	...
	if (current) {
		for( ;
			 (current) && current != tree;
			 (current) = VTREE_preorder_next_ext(current, visit, context)) {
				 
			visit( context, VTREE_PREORDER, context );
		}
	}
...
V_INLINE VTREENODE * VTREE_find_postorder(VTREENODE *node, VTREE_VISITOR visit,void *context)
{
	VTREENODE *current;
 ...
 */
V_INLINE int VTREE_check_tree(VTREENODE *root)
{
